...
- **A finding delete is blocked while any `releases` row references it.** A release is a record that the finding is currently shown to students; deleting the finding while that row exists would cascade-destroy the release record. We refuse with the name of a run that currently has it released. Once the release row is removed by an un-release, the finding may be deleted.
