...
Supabase Broadcast is fire-and-forget with no replay: a ping sent while the viewer's websocket is (re)connecting is lost forever. Publishing is done over a single awaited REST call (`channel.httpSend`, see `lib/broadcast.ts`) instead of a WebSocket join, so a dedicated page reload isn't needed — but if the viewer's socket happens to be down at that instant, the ping is lost. A viewer opening the page *after* the release simply fetches current state, so it is never stale. If a viewer was live during the release and the broadcast genuinely failed, reload the viewer page.

**Viewer never updates at all on the live app (not even eventually):**
Check `NEXT_PUBLIC_SUPABASE_URL` in Vercel. It must be the project **root** URL (`https://<ref>.supabase.co`), not the Data API URL shown on the Supabase "API" page (`https://<ref>.supabase.co/rest/v1`) which is a common copy-paste mistake. Both the browser viewer's websocket and the server-side publisher (`lib/broadcast.ts`, via `channel.httpSend`) build their Realtime endpoints from this value, so a `/rest/v1` suffix breaks Realtime entirely while HTTP still works. `lib/env.ts` now rejects the bad value at build time; fix the variable and redeploy.
